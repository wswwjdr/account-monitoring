...
在本机管理账号。当前版本导入 Outlook 四段凭证（邮箱、密码、Client ID、刷新令牌），用微软 Refresh Token 换 Access Token，经 Microsoft Graph 查看收件箱。
...
每行一个账号，四个字段必须完整，用 Tab 或 `----` 分隔：

```
邮箱地址----密码----Client ID----刷新令牌
```

- 邮箱不重复：直接插入到所选分组（可不选，即为未分组），写入导入时间。
- 邮箱已存在：选择「追加新版本」或「覆盖该邮箱最新一条」；本次都会使用所选分组。
- 「清空后导入」会先删除全部账号再写入，分组本身保留。
